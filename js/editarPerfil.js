 ...
 $(document).ready(function () {
    let monthNames = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']; //Defino mi arreglo de meses
    ...
    function funcMostrarModal(){
            ...
            .done(function (data) {
                ...
                $(".misHoras").filter(function() {
                    return $(this).text() === items[i].hora;
                }).parent().parent().children(".miAsesorAsignado").attr("id", items[i].Usuario_id);

                $(".misHoras").filter(function() {
                    return $(this).text() === items[i].hora;
                }).parent().parent().attr("id", items[i].Asesoria_id);
    ...
    if(miIdActual == miUserAsesoria){
        if(confirm("¿Desea eliminar esta asesoria?")){ // Si confirmo entro a el if ya que e valor de retorno es un booleano
        ...
        .done(function () {
            ...
            }).children(".miEstado").text("No hay asesorias asignadas");
            
            $(".list-group-item").filter(function() {
                return $(this).attr("id")  === miIdAsesoria;
            }).children(".miAsesorAsignado").text(" ");

            $(".list-group-item").filter(function() {
                return $(this).attr("id")  === miIdAsesoria;
            }).children(".d-flex").children(".text-muted").children(".badge").text("Disponible");

            $(".list-group-item").filter(function() {
                return $(this).attr("id")  === miIdAsesoria;
            }).children(".d-flex").children(".text-muted").children(".badge").removeClass("bg-secondary");

            $(".list-group-item").filter(function() {
                return $(this).attr("id")  === miIdAsesoria;
            }).children(".d-flex").children(".text-muted").children(".badge").addClass("bg-success");

            $(".list-group-item").filter(function() {
                return $(this).attr("id")  === miIdAsesoria;
            }).children(".miAsesorAsignado").attr("id", " ");
...
$.ajax({ //Hago mi llamada asincrona con AJAX (para no recargar la pagina)
    url: 'php/userAPI.php',
    type: 'POST',
    cache: false,
    contentType: false,
    ...
    function funcGuardarAsesoria()
{
  ...
  if(nombreMateria != "" && Descripcion != "" && Lugar != "" && Costo != ""){ //Verifico que mis datos esten completos para Seguir con el programa
    ...
    $.ajax({ // LLamada asincrona para registro de asesorias
        url: 'php/asesoriaAPI.php',
        type: 'POST',
        cache: false,
        contentType: false,
