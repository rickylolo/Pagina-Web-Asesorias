   ...
   END IF;
    ...
    THEN 
    	SET sp_nombres=IF(sp_nombres='',NULL,sp_nombres),
            ...
            sp_semestre=IF(sp_semestre='',NULL,sp_semestre),
			sp_matricula=IF(sp_matricula='',NULL,sp_matricula),
			sp_contraseña=IF(sp_contraseña='',NULL,sp_contraseña),
		...
		UPDATE Usuario 
			...
			semestre= IFNULL(sp_semestre,semestre), 
			matricula= IFNULL(sp_matricula,matricula), 
			contraseña= IFNULL(sp_contraseña,contraseña),
            fotoPerfil= IFNULL(sp_fotoPerfil,fotoPerfil)
		WHERE Usuario_id=sp_Usuario_id;
   END IF;
	IF Operacion = 'L' THEN /*INICIO DE SESION USUARIO*/
	SELECT Usuario_id,matricula,fotoPerfil,CONCAT(nombres,' ',apellidos) AS nombreCompleto FROM Usuario WHERE matricula = sp_matricula AND contraseña = sp_contraseña;
   END IF;
END //
